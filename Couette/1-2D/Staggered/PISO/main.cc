...
void PISO(void)
{
	...
	for (int i = 3; i <= Nx; ++i)
		for (int j = 2; j <= Ny; ++j)
		{
			...
			double dpdy = (p(i - 1, j) - p(i - 1, j - 1)) / dy;
	...
	for (int j = 2; j <= Ny - 1; ++j)
		for (int i = 2; i <= Nx; ++i)
		{
			double dpdx = (p_star1(i, j) - p_star1(i - 1, j)) / dx;
	...
	for (int i = 3; i <= Nx; ++i)
		for (int j = 2; j <= Ny; ++j)
		{
			double dpdy = (p_star1(i - 1, j) - p_star1(i - 1, j - 1)) / dy;
	...
	for (int j = 2; j <= Ny - 1; ++j)
		for (int i = 2; i <= Nx; ++i)
		{
			...
			u_wedge(i, j) = (rho * u(i, j) + A * dt - dt / dx * (p_star(i, j) - p_star(i - 1, j))) / rho;
		}
	...
	for (int i = 3; i <= Nx + 1; ++i)
		for (int j = 2; j <= Ny; ++j)
		{
			...
			v_wedge(i, j) = (rho * v(i, j) + B * dt - dy / dy * (p_star(i - 1, j) - p_star(i - 1, j - 1))) / rho;
		}
